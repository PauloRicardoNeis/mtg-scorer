...
def download_default_cards_snapshot(raw_root: Path) -> ScryfallSnapshot:
    ...
    staging = Path(tempfile.mkdtemp(prefix=".download-", dir=raw_root))
    try:
        temporary_file = staging / raw_file.name
        request = Request(
            download_uri,
        ...
        content_digest = _file_sha256(temporary_file)
        ...
        try:
            ...
            if not manifest_file.exists() or not raw_file.exists():
                raise
            manifest = json.loads(manifest_file.read_text(encoding="utf-8"))
            if (
                _file_sha256(raw_file) != content_digest
                or manifest.get("content_sha256") != content_digest
            ):
                raise ValueError("concurrent source snapshot conflict") from None
        return _snapshot_from_manifest(manifest, raw_file, manifest_file)
    finally:
        if staging.exists():
...
def normalize_snapshot(
    ...
    *,
    ...
    if _file_sha256(snapshot.raw_file) != snapshot.content_sha256:
    ...
    staging_dir = Path(tempfile.mkdtemp(prefix=".normalize-", dir=output_dir.parent))
    staged_cards = staging_dir / oracle_cards_file.name
    staged_printings = staging_dir / printings_file.name
    ...
    manifest = {
        ...
        "oracle_cards_sha256": _file_sha256(staged_cards),
        "card_printings_sha256": _file_sha256(staged_printings),
    ...
    try:
        ...
        try:
            ...
            if not silver_manifest_file.exists():
                raise
            return normalize_snapshot(snapshot, output_root=output_root)
    finally:
...
def _materialize_tables(connection: duckdb.DuckDBPyConnection) -> None:
    connection.execute(
        ...
        QUALIFY row_number() OVER (
            ...
            ORDER BY (language = 'en') DESC NULLS LAST, released_on DESC NULLS LAST, scryfall_id
        ) = 1
        """
    )
    connection.execute(
